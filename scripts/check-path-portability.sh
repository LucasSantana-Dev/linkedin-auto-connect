...
mode="${1:-show}"
if [ "$mode" != "show" ] && [ "$mode" != "json" ] && [ "$mode" != "--json" ]; then
...
if [ "$parent_name" = "forge-space" ]; then
  legacy_root="/Users/lucassantana/Desenvolvimento/forge-space/$repo_name"
  legacy_home="\$HOME/Desenvolvimento/forge-space/$repo_name"
else
  legacy_root="/Users/lucassantana/Desenvolvimento/$repo_name"
...
check_pattern_absent() {
  ...
  rg -n --hidden \
    ...
    --fixed-strings -- "$pattern" "$repo_root" >"$hits_file" || true
